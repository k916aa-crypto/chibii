...
app.post('/generate', async (req, res) => {
  try {
    const { skin, hair, outfit, accessory, background } = req.body;
    ...
    const prompt = `Cute chibi anime character with ${skin}, ${hair}, wearing ${outfit}, with ${accessory}, set in ${background}. Chibi proportions with very large round head and tiny body (3:1 ratio), big expressive glowing purple anime eyes with highlights, black Nike Air Jordan sneakers, purple and cyan energy aura glowing around the body, dark cyberpunk streetwear aesthetic, vibrant neon colors, dramatic rim lighting. High quality anime manga illustration style, masterpiece, ultra detailed.`;

    console.log('✦ Prompt:', prompt);

    // ── Replicate — FLUX Schnell (~$0.003/image, ~5s) ────────────────────────
    ...
    if (!replicateRes.ok) {
      const e = await replicateRes.json().catch(() => ({}));
      throw new Error(e.detail || `Replicate error ${replicateRes.status}`);
    }
    ...
    // Poll if Prefer:wait didn't resolve immediately
    ...
    // Proxy image back as base64 — avoids CORS issues in browser
